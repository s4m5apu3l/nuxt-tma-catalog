			...
			>
				<div class="flex flex-col items-center text-center space-y-2 py-2">
					...
					<div>
						...
						<p class="text-xs font-medium text-muted-foreground mt-0.5">{{ card.title }}</p>
					</div>
				</div>
			</UCard>
		</div>
		...
		<UCard>
			<template #header>
				<div class="flex items-center space-x-2">
					...
					<h3 class="text-base font-semibold">{{ t('admin.dashboard.quickActions') }}</h3>
				</div>
			</template>
			...
			<template #header>
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-2">
						...
						<h3 class="text-base font-semibold">{{ t('admin.dashboard.recentProducts') }}</h3>
					</div>
					<UButton to="/admin/products" variant="ghost" size="xs" icon="i-lucide-arrow-right" trailing>
						{{ t('common.view_all') }}
					</UButton>
				</div>
			</template>
			...
			<div class="space-y-2">
				...
				<template v-else-if="recentProducts.length > 0">
					<div
						...
						:key="product.$id"
						class="flex items-center space-x-3 p-2 rounded-lg active:bg-muted/50 transition-colors cursor-pointer"
				...
				<template v-else>
					<div class="text-center py-6 text-muted-foreground">
						<UIcon name="i-lucide-package" class="w-10 h-10 mx-auto mb-2 opacity-50" />
						<p class="text-sm">{{ t('admin.dashboard.noRecentProducts') }}</p>
					</div>
				</template>
			</div>
		</UCard>
